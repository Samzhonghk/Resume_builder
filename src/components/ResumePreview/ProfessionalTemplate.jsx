...
function SectionHeader({ title }) {
  return (
    <div className="mb-2">
      <h2 className="font-bold uppercase text-gray-900 text-center" style={{ fontSize: '9px', letterSpacing: '0.15em' }}>
        {title}
      </h2>
  ...
  return (
    <div className="bg-white px-8 py-8 text-gray-900" style={{ fontFamily: 'Arial, Helvetica, sans-serif' }}>
      <header className="text-center mb-2">
        <h1 className="font-bold uppercase leading-tight text-gray-900 tracking-widest" style={{ fontSize: '20px' }}>
          {name || <span className="text-gray-300">Your Name</span>}
        </h1>
        {contactParts.length > 0 && (
          <p className="mt-1.5 text-gray-500" style={{ fontSize: '11px' }}>{contactParts.join(' · ')}</p>
        )}
      </header>
            ...
            {workExperience.map(entry => {
              ...
              return (
                <div key={entry._id}>
                  ...
                  {entry.title && (
                    <div className="italic text-gray-600 leading-snug" style={{ fontSize: '11px' }}>{entry.title}</div>
                  )}
          ...
          <div className="space-y-3">
            {education.map(entry => (
              <div key={entry._id}>
                ...
                {entry.degree && <div className="text-gray-600 leading-snug" style={{ fontSize: '11px' }}>{entry.degree}</div>}
              </div>
            ))}
          </div>
        </section>
